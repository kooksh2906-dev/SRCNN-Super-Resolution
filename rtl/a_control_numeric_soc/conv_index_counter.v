...
module conv_index_counter (
    ...
    always @(*) begin
        last_index_o =  ({1'b0, out_channel_o} == (out_channel_count_i - 7'd1)) &&
                        ({1'b0, out_y_o} == (output_size_i - 6'd1))             &&
                        ({1'b0, out_x_o} == (output_size_i - 6'd1))             &&
                        ({1'b0, in_channel_o} == (in_channel_count_i - 7'd1))   &&
                        (kernel_y_o == (kernel_size_i - 4'd1))                  &&
                        (kernel_x_o == (kernel_size_i - 4'd1));
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            else if (kernel_x_o == (kernel_size_i - 4'd1)) begin
                kernel_x_o <= 4'd0;

                // kernel_y가 끝나면 0으로 복귀하고 in_channel로 Carry
                if (kernel_y_o == (kernel_size_i - 4'd1)) begin
                    kernel_y_o <= 4'd0;

                    // 모든 Kernel 순회 후 다음 Input Channel로 이동
                    if ({1'b0, in_channel_o} == (in_channel_count_i - 7'd1)) begin
                        in_channel_o <= 6'd0;

                        // 모든 Input Channel 순회 후 다음 Output X로 이동
                        if ({1'b0, out_x_o} == (output_size_i - 6'd1)) begin
                            out_x_o <= 5'd0;

                            // 한 행이 끝나면 다음 Output Y로 이동
                            if ({1'b0, out_y_o} == (output_size_i - 6'd1)) begin
                                out_y_o <= 5'd0;
